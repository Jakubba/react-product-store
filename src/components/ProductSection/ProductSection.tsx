...
const ProductSection: React.FC<ProductSectionProps> = ({ title, products, loading, showMore = false, pageSize = 10, lastProductRef }) => {
  ...
  return (
    <div className="my-8 max-w-[960px] mx-auto">
      {loading && products.length === 0 ? <SkeletonTitle /> : <h2 className="text-xl font-bold mb-4">{title}</h2>}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 md:gap-6">
        {loading && products.length === 0
          ...
          : visibleProducts.map((product, index) => {
              ...
              return <ProductCard key={product.id} product={product} ref={isLast ? lastProductRef : undefined} />
            })}
      </div>
